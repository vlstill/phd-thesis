...
This technique is often called \emph{Counterexample-Guided Abstraction Refinement} (\emph{CEGAR}) \mcite{Clarke2000}.
...
\begin{widefigure}
  ...
  \caption[A push in a lock-free queue]{A push (enqueue) operation of a single-producer-single-consumer
    ...
    API for this operation) or a complete redesign of the queue block.
...
standard, as defined by its latest working draft N4860~\mcite{cpp20}.\mnote{The
...
C++ comes with various high-level synchronisation primitives.\mnote{They are
usually based on operating system level blocking primitives -- i.e., a thread
waiting on these synchronisation primitives is suspended and not using any
resources.}
...
\paragraph{Atomic Variables and Low-Level Synchronisation}
%
...
have thrown it until it triggers a \cpp{catch} code block that can catch it --
...
\paragraph{Threads and High-Level Synchronisation in LLVM}
...
be inspected or caught in the functions that perform the \li{call}.
...
\li{invoke}.
If an exception is propagated through \li{invoke}, it transfers control
...
To execute the program, DIVINE uses \divm, a verification-oriented
...
On top of the file system support, DIVINE also has support for other parts of the POSIX API, including parts of networking and clock API \mcite{RBMKB2019}.
