...
be generalized to multiple cooperating programs, but the distinction is mostly
...
\paragraph{Lock-Based Synchronization}
...
threads while other thread executes the critical section, which can increase
performance of systems with more threads then available processors as the
waiting threads are completely inactive and other threads can be executed in
the meanwhile.
...
and the overhead of threads and their synchronization will make the program
slower then its sequential version.
...
\begin{widefigure}
  ...
  \caption[A push in a lock-free queue]{A push (enqueue) operation of a single-producer-single-consumer
    ...
    atomic operations).
    The queue uses a linked list of blocks (of type \texttt{Node}), each block
    can hold a fixed number of elements of some type~\texttt{T}.
    ...
    synchronization unnecessary on x86-64 processors.
    ...
    x86 and to prevent reordering by compiler -- the release ordering ensures
    ...
    atomic step (which is not possible on most processors and there is no C++
    API for this operation) or complete redesign of the queue block.
  }\label{fig:prelim:lockfree}
\end{widefigure}

Overall, there is often a steep compromise between code simplicity and
performance.
A code that uses a few locks can be often reasonably understandable and the
...
In this work, we are mostly focusing on analysis of C++.
For this reason, we will now introduce some of the features of this programming language that are relevant to our work.
After that, we will introduce LLVM intermediate representation, which is a low-level programming language used by clang and other LLVM-based compiler.
DIVINE, the model checker in which the work presented in this thesis is implemented, actually analyses LLVM and uses the clang compiler to translate C++ to LLVM prior to the analysis.
...
resources.}
Two of the main synchronization primitives are \emph{mutexes} and \emph{condition variables}.
...
\texttt{wait} is a blocking call that suspend its calling thread until another
...
For integral types, \cpp{std::atomic<T>} defines various operations that allow
...
This allows implementation of atomic types for data types that are larger then the platform-specific limit of atomic instructions.
...
\begin{description}
    ...
    \item[\texttt{std::memory\_order::acquire}] is used with load operations.
        ...
        create a synchronization that ensures that all modifications
    ...
    \item[\texttt{std::memory\_order::relaxed}] is the weakest atomic
        ordering.
        It guarantees no synchronization with operations on other memory
        locations.
...
have throw it until it triggers a \cpp{catch} code block that can catch it --
a \cpp{catch} block that either catches exactly the type of the exception, or
if the class of the exception uses inheritance, the exception can also be
caught a by a \cpp{catch} block that catches some of the predecessor types of
the exception.
...
One of the core features of C++ is that is has deterministic destruction of objects, i.e., the end of lifetime of objects is precisely known (as opposed to being determined by garbage collector in garbage-collected languages such as Java or C\#).
In C++, the end of life of an object happens either when it goes out of scope (for scope-allocated objects) or when it is explicitly deallocated (if the object was dynamically allocated).
This in turn allows C++ destructors, which are special functions executed at the end of life of an object, to be used for resource control.
For example a \texttt{std::unique\_lock} object might be initialised with an instance of a mutex and it will automatically lock the mutex when the \texttt{unique\_lock} is constructed and release the mutex when it goes out of scope.
...
An important property of this approach to resource management is that it is automatically exception safe -- if the resource-owning object is a local variable, the resource will be automatically release at the end of its scope, whether or not the end of scope was caused by exception propagation or normal control flow.
...
compilers.
The compiler build on LLVM consists of a language-specific frontend that
processes the source code and produces a language-independent LLVM intermediate
representation, an optimizer that runs on the LLVM intermediate
...
There are two more instructions that can access memory, and these are used for
...
be inspected or caught in the functions that performs the \li{call}.
...
concurrency and hard to discover bugs \mcite{DIVINEToolPaper2017,DIVINEWeb}.
...
(the stack layout is not guaranteed b the C/C++ standards and therefore such
...
\begin{tikzpicture}[ auto, node distance=3cm
                   ...
                   , half/.style={ minimum width = 8.75em,
    ...
    \node[state, shaded, below = 0em of libs.south west, anchor = north west]
        (dios) {\dios};
    \node[state, below = 0em of dios.south west, anchor = north west, half]
        (divm) {\divm};
    \node[state, below = 0em of dios.south east, anchor = north east, half]
...
\divm hypercalls behave as functions from the point of view of LLVM IR, but
they are treated as instructions by \divm.
...
Therefore, DIVINE needs substantial control over the compilation process, for example it is not allowed to use system-installed (binary) libraries with DIVINE.

To make compilation reasonably simple for users, DIVINE integrates the clang compiler using its library interface.
This integrated compiler compiles the program in an environment that contains only the libraries available in DIVINE, automatically links them, and produces LLVM IR that can be directly analysed with DIVINE.
The compiler can be invoked either automatically by DIVINE when it is executed with a C or C++ file, or there is a standalone compiler that can be used in place of clang.
...
The C and C++ standard libraries are mostly based on existing projects (that aim at normal execution), namely PDClib for the C library and libc++ and libc++abi for the C++ library.

\paragraph{File System and POSIX API}

C and C++ standard libraries provide, among other, access to the file system.
To be able to use file system in programs analysed by DIVINE, DIVINE provides two possible approaches.
...
The program can work with the file system as normally, but this has no effect on the file system of the computer the program runs on, instead the file system is part of the program's state.
The file system can either start with an empty snapshot, or it can load a snapshot of a directory at start.

An alternative approach is to actually allow the program to access file system of the machine.
This, however, is not possible during normal state space exploration that explores the state space in BFS-like order as it could cause interference between independent parts of the state space.
Instead, DIVINE can execute a single run of the program (giving random values to nondeterministic choices), record all supported system calls, and then use this recording in analysis \mcite{KRB2017}.
...
On top of the file system support, DIVINE has also support for other parts of the POSIX API, including parts of networking and clock API \mcite{RBMKB2019}.

\subsection{Program Instrumentation}

To avoid unnecessary complexity in \divm, DIVINE uses program instrumentation
as a preprocessing step.
This steps runs on the LLVM IR of the program and all the libraries linked to it.
Instrumentation adds potential interruption points to the program to specify point at which the scheduler can be invoked.
...
memory can interfere with other threads in LLVM.\mnote{In LLVM these are
\li{load}, \li{store}, \li{cmpxchg}, and \li{atomicrmw}.}
