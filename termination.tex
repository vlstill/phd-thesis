...
\caption{
    ...
    Please note that in C++ it is possible to use scope-based locks: the critical section belonging to mutex \lstinline{m} is entered when \lstinline{unique_lock lock(m)} is executed and left at the end of the scope in which the \lstinline{lock} variable was defined (at the matching curly brace; also marked with comment \lstinline{// unlock}).
} \label{fig:ntermsec0}
\end{minipage}
%
\hfill
...
\end{lstlisting}
\caption{
    A fixed version of the program from \autoref{fig:ntermsec0} (the start of the critical section was moved from the position \lstinline{// Error} in the left code to \lstinline{// Fixed} and therefore the critical section can end now).
