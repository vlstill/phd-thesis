...
\mcite[Chapter 5]{Baier2008} and Computation Tree Logic (CTL) \mcite[Chapter 6]{Baier2008} in the context of model checking dates way back to the beginning of research in formal methods.
...
There are also techniques that detect deadlocks of the whole program (i.e., a program state from which the program cannot move) \mcite{Chaki2005,Demartini99}, but these techniques cannot find cases in which only some threads of the program are making progress, while other threads are blocked forever.
...
% SACO a tool for analysis of concurrent programs with loops is presented in \mcite{Albert2017}.
% It uses may-happen-in-parallel analysis to infer which part of the program can interfere and it also also infers upper bounds on iteration numbers.
% It targets a modelling language which contains shared memory.
% It generates constraints on how the shared state can be modified in order for the loop to terminate and than checks that this will eventually hold.
% Arctor \mcite{Kupriyanov2014} is another tool for termination detection of parallel programs written in a simple modelling language.
% It uses abstraction and refinement and its termination checking is based on well-founded relations.
% It can show termination, but cannot prove non-termination.
...
Another tool for termination-checking parallel C programs is presented in \mcite{Popeea2012}.
...
Probably the closes relative to our approach is shown in \mcite{Atig2012term}, which presents a tool MUTANT that can detect fair nontermination.
