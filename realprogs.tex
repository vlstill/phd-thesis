...
DIVINE.
The evaluation is unmodified from the original paper.}
...
languages.
For example, C has dynamic memory, pointer arithmetic,\mnote{Using pointer
arithmetic it is possible to access elements of arrays and data structures by
numeric manipulation of pointer values.} and function pointers.
...
\mnote{Templates allow the programmer to write a generic code which
    does not depend on concrete types, the compiler is then responsible for
    the generation of implementations for concrete types the code is used with.}
inheritance with multiple base classes, runtime type information (RTTI),
compiler-generated special member functions,%
\mnote{C++ compiler can auto-generate code that handles copying, creation and
    destruction of objects, and since C++20 also comparison of objects. Of
    course, the programmer can override the default behaviour if it is not
...
Considering the efforts put into compiler development and the complexity of
...
\autoref{sec:lang:except} then takes a more detailed look at the particular case of exception support in DIVINE 4; this part was published originally in \mcite{SRB2017}.
...
Reuse of execution-oriented components (such as compilers and libraries) for
...
Many program analysis tools are in fact analysing LLVM IR and not C/C++ directly (for example DIVINE \mcite{DIVINEToolPaper2017}, Skink \mcite{Cassez2017}, VVT \mcite{Gunther2016}, Symbiotic \mcite{Chalupa2018}, and many more).
...
ESBMC \mcite{Gadelha2018}, which uses only the C and C++ parser from clang and
...
The advantages and disadvantages of library reuse are in part similar to the
...
library.\mnote{An example is a data race in initialization of C++ threads in
libc++ discovered by DIVINE.
...
Overall, we believe that reuse of (parts of) existing library implementations is crucial for good language support.
...
Examples of these tools are ESBMC++ \mcite{Ramalho2013} for C++, SMACK for C, C++, Rust and many other \mcite{Garzella2020}, and Java Path Finder \mcite{Artho2019} for Java, all of which mention that they support only parts of the respective standard libraries.
...
matching the naming introduced by~\mcite{DIVINEToolPaper2017}.
...
are built at the time of build of DIVINE.
The original integration of clang into DIVINE 4 and the adaptation of LLVM
linker for use with DIVINE was done by me.
The work was then taken over by other members of the team.
The current state of the compilation for DIVINE is described in~\mcite{BR2020}.

To provide support for most of C and C++, DIVINE has to provide standard libraries for these languages.
...
DIVINE also has support for many POSIX standard functions, including large parts of the file system API \mcite{RBMKB2019}.
...
implemented for use with \divine or other program analysis tools \mcite{RBMKB2019}.
...
Exceptions in C++ are among the features that are both widely used (also by the standard library) and tricky to implement.
Their use is, however, also common outside of the standard library: libraries
like boost\mnote{Boost is one of the most used collections of general-purpose C++ libraries.
Many features of boost eventually get into the C++ standard.}
...
for nondeterminism to rigorously explore error paths.\mnote{This is a
  ...
  nondeterministically). The model checker will then take care of exploring
  all possible combinations of succeeding and failing memory allocations in the
  program.}

\subsection{Contribution}\label{contribution}

The approach to exception support described in this section, and originally published in \mcite{SRB2017} brings the following contributions:
...
All source code related to this work, along with supplementary material, is available online under a permissive open-source licence.\mnote{\url{https://divine.fi.muni.cz/2017/exceptions}}
...
comprehensive enough\mnote{That is, some of the less frequently used
  ...
  all. That is to say, the size of the libc++abi portion that
  would have needed to be reimplemented was initially underestimated.}
...
the natively executing program.\mnote{Clearly, the libunwind
  ...
  libunwind must be established separately.}
...
library.\mnote{There are many implementations of the C++ runtime
  ...
  Each implementation is usually tied to a particular implementation of the C++
  standard library.
  Commonly used implementations on Unix-like systems are libsupc++,
  which comes with libstdc++ and the GCC compiler, and
  libc++abi, which is tied to libc++ used by some builds of
  clang and by \divine{}.}
...
\begin{tikzpicture}[ ->, >=stealth', shorten >=1pt, auto, node distance=3cm
    ...
    \draw (vmreg-1-3) edge[out=220, in=47] (close)
        ...
        (exception-4-1.east) to[out=45, in=-45] (unwind-1-1.east)
...
is not caught.\mnote{Section~14.4, paragraph~9 of the C++20 standard \cite{cpp20}}
...
\texttt{landingpad} that is about to be executed.\mnote{Please note that LLVM registers are function-local values that cannot be used to pass information between functions in pure LLVM.
With \divm, we can use metadata to locate the register's value in a frame of a function and modify it.}
...
support for C++. LLBMC~\mcite{Falke2013} and
NBIS~\mcite{Gunther2014} are \llvm{}-based bounded model
checkers which target mainly C and have no support for exceptions or the
C++ standard library. VVT~\mcite{Gunther2016} is a
...
Furthermore, KLEE~\mcite{Cadar2008} and KLOVER~\mcite{Li2011}
are \llvm{}-based tools for test generation and symbolic execution. KLOVER
targets C++ and according to~\mcite{Li2011} has exception support,
...
Both CBMC~\mcite{Clarke2004,Kroening2014} and
ESBMC~\mcite{Gadelha2018} bounded model checkers support C++
...
of the \cpp{try-catch} syntax and throwing an exception triggers an error.%
\mnote{A simple test which throws and tries to catch an exception object
crashes CBCM 5.11 during the verification and a code which attempts to catch an
exception by reference (which is the recommended practice in C++) results in
spurious syntax error during program parsing.}
...
specifications.\mnote{ESBMC 3.0 is unable to determine that an
...
Finally, \divine{} 3~\mcite{RBB16} also comes close to
...
constructs. This approach was taken in~\mcite{Prabhu2011},
...
Exceptions are more widely supported in the context of Java analysis, probably thanks to their prevalence in Java code.
Examples of tools for analysis of Java with exception support are Java Path Finder \mcite{Artho2019}, JBMC \mcite{Cordeiro2019}, and JDart \mcite{Mues2020}.
...
way.\mnote{In this case, the handler is installed using
