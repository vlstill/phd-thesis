...
\begin{tikzpicture}[ ->, >=stealth', shorten >=1pt, auto, node distance=3cm
                   ...
                   , state/.style={ rectangle, draw=black, very thick,
                     ...
                     sep=2pt, text centered, node distance = 1.9em }
                   ]
  \node[state, minimum width = 6em] (code) {C++ code};
  \node[state, minimum width = 10.4em, right = 11.7em of code] (prop) {property and options};
  ...
  \node[state, right = of link, rounded corners, minimum width = 8em] (lart) {instrumentation};
  \node[state, right = of lart.north east, anchor = north west, rounded corners, text width = 8em, align = center] (verifier) {verification core\\(\divm)};
  ...
  \node[state, left = 1em of valid, minimum width = 8em] (ce) {\color{red}Counterexample};
  ...
  \path (prop.348) edge[|*] (verifier.north)
        ...
        (verifier.310) edge[|*] (valid.north)
        (verifier.200) edge[|*] (ce.north)
        ;
\end{tikzpicture}
...
takes memory directly from \divm, and program startup and exit which is \dios specific.
...
\TODO{The apporach to exception support described in this section, and originally published in \cite{SRB2017} bringhs the following contributions:
...
language~\cite{RSCB2018} specifically, and will therefore only
...
verification~\cite{RBB16}, is to aid authors of
...
tool, either the approach described in~\cite{RBB16} or
...
comparison of the two approaches is given in \autoref{sec:cmpD3}.
...
time~\cite{RBB16}. However, while basic exception
...
specification~\cite{RSCB2018} and are generally useful, regardless
...
more efficiently~\cite{RSCB2018}. Additionally, this way the stack
...
\begin{tikzpicture}[ ->, >=stealth', shorten >=1pt, auto, node distance=3cm
                   ...
                   , vmreg/.style={common, minimum height=2em, minimum width = 3em}
    ...
    \matrix[frame, below = 3em of vmreg] (close) {%
        \cpp{close()} \\
        caller \\
        PC = … \\
        …\\};
    ...
    \draw (vmreg-1-3) edge[out=310, in=47, looseness=2] (close)
        ...
        (main-5-1.east) edge[out=0, in=180] (exception-1-1.west)
      ;
\end{tikzpicture}
...
reduction~\cite{RBB13} is enabled. However, many of
...
the approach described in~\cite{RBB16} in the context
...
in~\cite{RBB16}. In line with this expectation, we set
...
approach~\cite{RBB16}. We believe that this small
...
\divm{} can be found in~\cite{RSCB2018}.
...
in~\cite{RBB16}, we have found that with a suitable
...
the advent of the \divm{} specification~\cite{RSCB2018} and its
