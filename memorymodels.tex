...
that it itself encodes the relaxed memory behaviour, and after that it is
...
higher-level synchronization constructs such as mutexes, or whether to tap to the full power of the architecture and risk subtle unintuitive behaviour of relaxed memory accesses.
...
\begin{widefigure}
  ...
  \begin{tikzpicture}[ ->, >=stealth', shorten >=1pt, auto, node distance=3cm
    ...
    \draw (s0) edge node[left, xshift=-1em] {flush \be{x}{1}} (s1)
      ...
      ;
  \end{tikzpicture}
  \caption[An example parallel program and a fragments of its state space.]{
    An example parallel program and a fragments of its state space.
...
In practice, the appearance of relaxed behaviour is often not a problem, provided the overall behaviour of the data structure or algorithm matches desired specification.
