...
This approach is used for example by \mcite{Zhang2015} or in our previous work in~\mcite{SRB15weakmem}.
...
The \divine's operating system, \dios, is used to implement low-level threading as well as simulation of various filesystem APIs \mcite{DIVINEToolPaper2017}.
...
From the theoretical point of view, we know due to Atig et al.~\mcite{wmdecidability} that reachability for programs with finite-state threads which run under TSO is decidable, but non-primitive recursive (it is in \textsc{pspace} for sequential consistency).
...
We compared our implementation with the stateless model checker Nidhugg~\mcite{Abdulla2015} and the bounded model checker CBMC~\mcite{Clarke2004}, \mcite{Kroening2014}.
