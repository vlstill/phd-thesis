...
\usepackage{pgf}
\usepackage{tikz}
\usetikzlibrary{arrows,positioning,shapes,snakes,automata,backgrounds,petri,fit,matrix,decorations.pathreplacing,calc}
\tikzset{
    *|/.style={
        to path={
...
\newcommand\fs@botruled{%
    \def\@fs@cfont{\bfseries}\let\@fs@capt\floatc@botruled%
...
\newcounter{thesis@c@fig@def}
\newcounter{thesis@c@fig@use}
\gdef\thesis@page@float#1{\relax}
\gdef\at@float@page{%
    ...
    \immediate\write\@auxout{%
        \noexpand\expandafter\noexpand\gdef\noexpand\csname thesis@at@figure@body@hook\thethesis@c@fig@def\noexpand\endcsname{\unexpanded{\changetext{}{\marginparwidth+\marginparsep}{}{}{}}}%
    }%
%    \expandafter\gdef\csname \endcsname{}
}%
\let\thesis@old@figure\figure
\let\endthesis@old@figure\endfigure
\let\thesis@orig@xfloat\@xfloat
\def\@xfloat #1[#2]{%
        \immediate\stepcounter{thesis@c@fig@def}%
        \thesis@orig@xfloat #1[#2]%
    }
\renewenvironment{figure}[1][tp]{%
        \begin{thesis@old@figure}[#1]%
        \stepcounter{thesis@c@fig@use}%
        \csname thesis@at@figure@body@hook\thethesis@c@fig@use\endcsname%
        \immediate\write\@auxout{%
            \noexpand\expandafter\noexpand\gdef\noexpand\csname xxx@thesis@at@figure@body@hook\thethesis@c@fig@use\noexpand\endcsname{}}%
        \@getfpsbit 8\relax
        \ifodd \@tempcnta
            p
        \else
...
\newcommand{\geom}[2]{\csname #1geometry\endcsname{
        bindingoffset=2cm, inner=1.5cm, outer=1.5cm, top=1.5cm, bottom=2.5cm,
%        headheight=15.2pt, % for the sake of fancyhdr
        includehead, headsep=1cm, nofoot,
        includemp, % count outer margin outside of the marginpar
        #2
    }}
\newcommand{\widegeom}{\geom{new}{nomarginpar}}
\newcommand{\margingeom}{\geom{new}{marginparsep=0.8cm, marginparwidth=3.5cm}}
...
\fancyhead[LE]{\footnotesize\makebox[\widthof{000}]{\thepage}\leftmark}
\fancyhead[RO]{\footnotesize\rightmark\quad \ \makebox[\widthof{000}]{\hfill\thepage}}
