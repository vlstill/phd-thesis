...
introduced.
These techniques include \emph{hash compaction} and \emph{bitstate hashing}~\cite{Holzmann1998}, which is an
...
\emph{lossless compression techniques}~\cite{RSB15TC,Laarman2019}.
...
Java Pathfinder (JPF)~\cite{Artho2019}, \cite{Visser2003}
- java bitecode programs (with limited support to call native code), including concurrency
- based on explicit-state model checking
- extension to symbolic execution Symbolic Pathfinder~\cite{Pasareanu2013}
  - test generation, assertion violations, concurrency errors in programs with inputs
  - targets unit tests and sub-system level testing
  - possibility of unit preconditions, combination of symbolic and concrete values
  - uses JPF to explore different interleavings
  - depth bound (state comparison disabled by default, if enabled disregards symbolic data)
  - support for symbolic input data structures -> lazily initialized and extended when needed (heap allocations)
- checks for uncaught exceptions (includes assertions in Java)
- uses hash-compaction for representation of the set of visited states~\cite{TODO} (-> under-approximation)
- groups instructions with local effects (~ $\tau$) with heuristics to decide which instructions can be grouped -- according to the authors it can miss behaviours
- nondeterministic choices - scheduling, data (over given range)
- partial model of the standard library
- some support for IO ??
...
Lazy-CSeq~\cite{Inverso2015}, \cite{Nguyen2017}
...
- error checking is deffered to the backend + deadlocks in POSIX thread mutex use
...
- abstract interpretation (using Frama-C~\cite{Canet2009}) added to infer variable bounds and the shrink bitvector variables in the formulas \cite{Nguyen2017}
